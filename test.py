...
selected = option_menu(
            menu_title=None,  
            options=["Suggest Cars", "Predict Price of Used Car", "Electric car Analysis","Electric vs Conventional Car"], 
            default_index=0, 
...
if selected == "Electric car Analysis":
    col7, col8 = st.columns([3, 3])
    with col7:
        #graph 1 
        #interactive graph displaying how sales for different makes varies year by year
        #contains data for 2010-2020

        sales= pd.read_excel(r"C:\Users\Anusha\Desktop\project\sales.xlsx")
        
        make_options = sales['make'].unique().tolist()
        year_options=sales['year'].unique().tolist()

        #taking input for makes to be displayed in the graph
        make=st.multiselect('Which make would you like to see?', make_options, default=('FORD','TOYOTA','BMW','MERCEDES'))

        sales = sales[sales['make'].isin(make)]
        
        fig2 = px.bar(sales, x="make", y="sales", color="make",range_y=[0,200000], animation_frame="year", animation_group="make")
    ...
    with col8:
        ...
        #graph 2
        df2=pd.read_csv(r"C:\Users\Anusha\Desktop\project\cars_engage_2022.csv")
        df1=pd.read_csv(r"C:\Users\Anusha\Desktop\project\ElectricCar.csv")
        
        
        df2['Ex-Showroom_Price']=df2["Ex-Showroom_Price"].astype(str).str.replace("Rs. ","")
        df2['Ex-Showroom_Price']=df2["Ex-Showroom_Price"].astype(str).str.replace(".","")
        df2['Ex-Showroom_Price']=df2["Ex-Showroom_Price"].astype(str).str.replace(",","")
        df2['Ex-Showroom_Price']=df2["Ex-Showroom_Price"].astype(str).str.replace(",","")
        

        df1['ev_price']=df1["ev_price"].astype(str).str.replace("Rs. ","")
        df1['ev_price']=df1["ev_price"].astype(str).str.replace(".","")
        df1['ev_price']=df1["ev_price"].astype(str).str.replace(",","")
        

        df2['Ex-Showroom_Price'] = df2['Ex-Showroom_Price'].astype(float)
        df1['ev_price'] = df1['ev_price'].astype(float)

        df = pd.merge(df2, df1, on=['Body_Type'], how="inner")
        df.drop(df.columns.difference(['Ex-Showroom_Price','ev_price','Body_Type']), 1, inplace=True)
        df.dropna(axis=0,inplace=True)
        
        a=df.groupby(by=["Body_Type"]).mean() 
        ...
        b=pd.read_csv(r"C:\Users\Anusha\Desktop\project\graph2.csv")
        fig = go.Figure(data=[go.Bar(name='Normal Car Price', x=b['Body_Type'], y=b['Ex-Showroom_Price']),go.Bar(name='Electric Vehicle Price', x=b['Body_Type'], y=b['ev_price'])])
        st.plotly_chart(fig)
        ...
        df2['Ex-Showroom_Price'] = df2['Ex-Showroom_Price'].astype(float)
        df1['ev_price'] = df1['ev_price'].astype(float)
        df2.drop(df2.columns.difference(['Ex-Showroom_Price','Make']), 1, inplace=True)
        df1.drop(df1.columns.difference(['ev_price','Make']), 1, inplace=True)
        df1.dropna(axis=0,inplace=True)
        df2.dropna(axis=0,inplace=True)
    
        df1_gp=df1.groupby(by=["Make"]).mean() 
        df2_gp=df2.groupby(by=["Make"]).mean()
    
        

        df1_gp.to_csv("he1.csv")
        df2_gp.to_csv("he2_gp.csv")

        g1=pd.read_csv(r"C:\Users\Anusha\Desktop\project\he2_gp.csv")
        g2=pd.read_csv(r"C:\Users\Anusha\Desktop\project\he1.csv")

        df5 = pd.merge(g1,g2, on=['Make'], how="inner")
        
        #df.drop(df.columns.difference(['Ex-Showroom_Price','ev_price','Make']), 1, inplace=True)
        #df.dropna(axis=0,inplace=True)
        df_g3=df5.groupby(by=["Make"]).mean() 
    ...
    #b=pd.read_csv(r"C:\Users\Anusha\Desktop\project\graph2.csv")
    #fig = go.Figure(data=[go.Bar(name='Ex-Showroom_Price', x=b['Make'], y=b['Ex-Showroom_Price']),go.Bar(name='ev_price', x=b['Make'], y=b['ev_price'])])
    #st.plotly_chart(fig)
